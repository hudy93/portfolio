...
const Services = () => {
    return (
        ...
        <div className="container mx-auto">
            <h2 className='section-title mb-16 xl:mb-24 text-center mx-auto'>My Services</h2>
            ...
            {serviceData.map((service, index) => (
                <Card className='w-full mx-w-[424px] h-[300px] flex flex-col pt-16 pb-10 justify-center items-center relative' key={index}>
                    <CardHeader className='text-primary absolute -top-[60px]'>
                        <div className='w-[140px] h-[50px] bg-white dark:bg-background flex justify-center items-center'> {service.icon}</div>
                    </CardHeader>
                    <CardContent className='text-center'>
                        <CardTitle className='mb-4'>{service.title}</CardTitle>
                        <CardDescription className='text-lg'>{service.desc}</CardDescription>
                    </CardContent>
                </Card>
            ))}
            </div>
